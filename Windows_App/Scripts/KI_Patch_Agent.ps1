...
$RunLog = Join-Path $LogDir "KI_PATCH_AGENT_$Ts.txt"
$RawLog = Join-Path $LogDir "KI_PATCH_AGENT_RAW_$Ts.txt"
$PatchFile = Join-Path $LogDir "KI_PATCH_AGENT_$Ts.patch"
...
function Validate-Patch {
    ...
    foreach ($Line in $Lines) {
        ...
        if ($L -match "^(diff --git a/)(.+?)( b/)(.+)$") {
            $A = $Matches[2]
            $B = $Matches[4]
            Assert-AllowedPath $A | Out-Null
            Assert-AllowedPath $B | Out-Null
        }
...
LogLine "KI-Patch-Agent gestartet."
...
LogLine "Prüfe Patch mit git apply --check."

git -C $Root apply --check $PatchFile

LogLine "Patchprüfung erfolgreich. Wende Patch an."
...
LogLine "KI-Patch-Agent beendet."
