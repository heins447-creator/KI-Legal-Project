# UI03-1d OCR-Freigabeablauf AUTOLAUF
$ErrorActionPreference = "Stop"
$ROOT = "I:\KI_Legal_Project"
$PYTHON = "$ROOT\Tools\Python312\python.exe"
$RUNNER = "$ROOT\Scripts\python_runner\ui03_1d_ocr_freigabeablauf.py"
$CHECK = "$ROOT\Scripts\python_runner\check_ui03_1d_ocr_freigabeablauf.py"
$LOG = "$ROOT\Windows_App\Logs\UI03_1d_OCR_FREIGABEABLAUF_BERICHT.txt"
...
Write-Log "UI03-1d OCR-FREIGABEABLAUF AUTOLAUF START"
Write-Log "=========================================="
...
if ($ok1 -and $ok2 -and $ok3 -and $ok4) {
    ...
    git -C $ROOT add -A
    git -C $ROOT commit -m "UI03-1d OCR-Freigabeablauf vor Uebersetzung (CORE-11-konform)"
    Write-Log "Git-Commit erfolgt"
} else {
    Write-Log "AUTOLAUF NICHT VOLLSTAENDIG BESTANDEN"
}

Write-Log "UI03-1d OCR-FREIGABEABLAUF AUTOLAUF ENDE"
