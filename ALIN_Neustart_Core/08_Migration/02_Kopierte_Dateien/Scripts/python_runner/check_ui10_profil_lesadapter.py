...
# -*- coding: utf-8 -*-
"""
CHECK UI10 – Externer Profil-Leseadapter
Prueft: Config-Struktur, Freeze-Kompatibilitaet, Sperrregister, Runner-Existenz, Konsistenzregeln
"""
...
def main():
    global FEHLER, WARNUNGEN
    print("=" * 60)
    print("CHECK UI10 – Externer Profil-Leseadapter")
    print("=" * 60)
    
    cfg_pfad = "Config/ui10_profil_lesadapter_v1.json"
    runner_pfad = "Scripts/python_runner/ui10_profil_lesadapter.py"
    ...
    if cfg:
        ...
        # Jede Regel hat Schwere und Beschreibung
        regeln_ok = all(r.get("regel_id") and r.get("beschreibung") and r.get("schwere") for r in regeln)
        t("Alle Regeln haben ID+Beschreibung+Schwere", regeln_ok)
        
        # 6. Bereitgestellte Module
        module = cfg.get("bereitgestellte_module", [])
        t("Mindestens 5 Module", len(module) >= 5)
        
        # Pruefe freeze_status
        frozen = [m for m in module if m.get("freeze_status") == "eingefroren"]
        t("Mindestens 5 eingefrorene Module gelistet", len(frozen) >= 5)
        
        # Pruefe dass eingefrorene Module zukuenftig sind
        frozen_zukuenftig = all(m.get("kompatibilitaet") == "zukuenftig" for m in frozen)
        t("Eingefrorene Module markiert als 'zukuenftig'", frozen_zukuenftig)
        
        # 7. Ausgabepfade
        ausgabe = cfg.get("ausgabe", {})
        t("json_adapter definiert", bool(ausgabe.get("json_adapter")))
        t("html_adapter_uebersicht definiert", bool(ausgabe.get("html_adapter_uebersicht")))
    ...
    if os.path.exists(sperrregister_pfad):
        try:
            ...
            ui10_eintraege = [e for e in eintraege if e.get("modul_id") == "UI10"]
            tw("UI10 im Sperrregister eingetragen", len(ui10_eintraege) > 0)
            if ui10_eintraege:
                e = ui10_eintraege[0]
                t("UI10 gesperrt (produktiv)", e.get("gesperrt") is True)
        except Exception as e:
    ...
    if os.path.exists(runner_pfad):
        try:
            ...
            spec = importlib.util.spec_from_file_location("ui10_runner", runner_pfad)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            t("Runner importierbar", True)
            t("lade_ui09_profil() definiert", hasattr(mod, "lade_ui09_profil"))
            t("lade_ui09_schalter() definiert", hasattr(mod, "lade_ui09_schalter"))
    ...
    print("\n" + "=" * 60)
    if FEHLER == 0:
        print(f"CHECK UI10 BESTANDEN – {WARNUNGEN} Warnung(en)")
    else:
        print(f"CHECK UI10 FEHLGESCHLAGEN – {FEHLER} Fehler, {WARNUNGEN} Warnung(en)")
    print("=" * 60)
