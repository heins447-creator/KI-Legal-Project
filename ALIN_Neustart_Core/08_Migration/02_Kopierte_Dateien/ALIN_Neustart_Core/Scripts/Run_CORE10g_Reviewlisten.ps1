...
Write-Host "CORE-10g: Reviewlisten dokumentieren" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
$compileErrors = 0
$scripts = @(
    "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10g_reviewlisten_dokumentieren.py",
    "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10g_pruefung.py"
)
...
& $PY "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10g_reviewlisten_dokumentieren.py"
$mainExit = $LASTEXITCODE

# 3. Prüfung
Write-Host "`n[3/4] Prüfung wird ausgeführt..." -ForegroundColor Yellow
& $PY "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10g_pruefung.py"
...
if ($compileErrors -eq 0 -and $mainExit -eq 0 -and $checkExit -eq 0) {
    Write-Host "CORE-10g: ALLES BESTANDEN" -ForegroundColor Green
} else {
    Write-Host "CORE-10g: FEHLER AUFGETRETEN" -ForegroundColor Red
