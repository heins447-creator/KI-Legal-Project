...
Write-Host "CORE-10d: Schnittstellenbereiche nachtragen" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
$compileErrors = 0
$scripts = @(
    "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10d_schnittstellenbereiche_nachtragen.py",
    "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10d_pruefung.py"
)
...
& $PY "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10d_schnittstellenbereiche_nachtragen.py"
$mainExit = $LASTEXITCODE

# 3. Prüfung
Write-Host "`n[3/4] Prüfung wird ausgeführt..." -ForegroundColor Yellow
& $PY "$ROOT\ALIN_Neustart_Core\Scripts\alin_core10d_pruefung.py"
...
if ($compileErrors -eq 0 -and $mainExit -eq 0 -and $checkExit -eq 0) {
    Write-Host "CORE-10d: ALLES BESTANDEN" -ForegroundColor Green
} else {
    Write-Host "CORE-10d: FEHLER AUFGETRETEN" -ForegroundColor Red
