...
Write-Host "Phase 0, AP 0.2 Migration-Runner starten" -ForegroundColor Cyan
...
$Runner = Join-Path $BaseDir "Scripts\python_runner\phase0_ap02_migration_runner.py"
$Check = Join-Path $BaseDir "Scripts\python_runner\check_phase0_ap02_migration_runner.py"

if ($Execute) {
    & $PythonExe $Runner --execute
} else {
    & $PythonExe $Runner --dry-run
}
if ($LASTEXITCODE -ne 0) {
    Write-Host "FEHLER: Migrations-Laeufer fehlgeschlagen." -ForegroundColor Red
    exit 1
}
...
Write-Host "Phase 0, AP 0.2 erfolgreich abgeschlossen" -ForegroundColor Green
