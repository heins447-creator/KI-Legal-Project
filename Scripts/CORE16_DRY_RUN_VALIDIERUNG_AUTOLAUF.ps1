...
.DESCRIPTION
    Prueft den Migrationsplan auf Kollisionen, Pfade und Sperrstatus.
...
$runner = Join-Path $projektWurzel "Scripts\python_runner\core16_dry_run_validierung.py"
$check = Join-Path $projektWurzel "Scripts\python_runner\check_core16_dry_run_validierung.py"

function Write-Log {
    param([string]$msg)
    Write-Host "[CORE-16] $msg"
}

Write-Log "Starte CORE-16 Dry-Run Validierung..."
...
Write-Log "CORE-16 DRY-RUN VALIDIERUNG ERFOLGREICH ABGESCHLOSSEN."
