...
$PyFile = Join-Path $Root "Scripts\python_runner\044_agent_sprache_uebersetzung_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_AGENT_SPRACHE_UEBERSETZUNG_$Ts.txt"
$Failed = $false
...
try {
    W "AGENT SPRACHE UND UEBERSETZUNG gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Agent Sprache und Übersetzung fehlgeschlagen."
    }

    W "AGENT SPRACHE UND UEBERSETZUNG abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Agent Sprache und Übersetzung beendet."
        exit 0
    }
}
