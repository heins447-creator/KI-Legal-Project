...
# 2. Vorab-Check
if (-not $SkipCheck) {
    Write-Header "2. Vorab-Check UI09"
    $checkScript = Join-Path $Base "Scripts\python_runner\check_ui09_zentrale_grundschalter_profilfelder.py"
...
# 3. Runner
if (-not $SkipRunner) {
    Write-Header "3. Runner UI09"
    $runner = Join-Path $Base "Scripts\python_runner\ui09_zentrale_grundschalter_profilfelder.py"
    if (Test-Path $runner) {
        python $runner
        if ($LASTEXITCODE -ne 0) {
...
# 4. Nach-Check
if (-not $SkipCheck) {
    Write-Header "4. Nach-Check UI09"
    $checkScript = Join-Path $Base "Scripts\python_runner\check_ui09_zentrale_grundschalter_profilfelder.py"
    python $checkScript
    if ($LASTEXITCODE -ne 0) {
...
$ausgaben = @(
    "Windows_App\Logs\UI09_ZENTRALE_GRUNDSCHALTER_BERICHT.txt",
    "Windows_App\Logs\UI09_ZENTRALES_PROFIL.json",
    "Windows_App\Logs\UI09_GRUNDSCHALTER.json",
    "Windows_App\Logs\UI09_PROFILUEBERSICHT.html"
)
...
# 6. Abschluss
Write-Header "UI09 AUTOLAUF ABGESCHLOSSEN"
