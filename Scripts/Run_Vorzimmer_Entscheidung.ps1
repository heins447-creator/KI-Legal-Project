...
$PyFile = Join-Path $Root "Scripts\python_runner\018_vorzimmer_entscheidung_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_VORZIMMER_ENTSCHEIDUNG_$Ts.txt"
$Failed = $false
...
try {
    W "VORZIMMER ENTSCHEIDUNG gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Vorzimmer-Entscheidung fehlgeschlagen."
    }

    W "VORZIMMER ENTSCHEIDUNG abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Vorzimmer-Entscheidungslauf beendet."
    }
}
