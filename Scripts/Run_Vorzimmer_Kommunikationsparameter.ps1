...
$PyFile = Join-Path $Root "Scripts\python_runner\035_vorzimmer_kommunikationsparameter_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_VORZIMMER_KOMMUNIKATIONSPARAMETER_$Ts.txt"
$Failed = $false
...
try {
    W "VORZIMMER KOMMUNIKATIONSPARAMETER gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Vorzimmer-Kommunikationsparameter fehlgeschlagen."
    }

    W "VORZIMMER KOMMUNIKATIONSPARAMETER abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Vorzimmer-Kommunikationsparameter sind geschrieben."
    }
}
