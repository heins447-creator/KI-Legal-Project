...
$PyFile = Join-Path $Root "Scripts\python_runner\033_posteingang_schlusskontrolle_v2.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_POSTEINGANG_SCHLUSSKONTROLLE_$Ts.txt"
$Failed = $false
...
try {
    W "POSTEINGANG SCHLUSSKONTROLLE gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if (-not (Test-Path -LiteralPath $PyFile)) {
        throw "Schlußkontrolle fehlt: $PyFile"
    }

    & $Python $PyFile 2>&1 | Tee-Object -FilePath $Report -Append

    if ($LASTEXITCODE -ne 0) {
        throw "Posteingang-Schlußkontrolle fehlgeschlagen."
    }

    W "POSTEINGANG SCHLUSSKONTROLLE abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Schlußkontrolle beendet."
    }
}
