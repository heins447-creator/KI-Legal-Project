...
# 2. Vorab-Check
if (-not $SkipCheck) {
    Write-Header "2. Vorab-Check UI11"
    $checkScript = Join-Path $Base "Scripts\python_runner\check_ui11_register_lesadapter.py"
...
# 3. Runner
if (-not $SkipRunner) {
    Write-Header "3. Runner UI11"
    $runner = Join-Path $Base "Scripts\python_runner\ui11_register_lesadapter.py"
    if (Test-Path $runner) {
        python $runner
        if ($LASTEXITCODE -ne 0) {
...
# 4. Nach-Check
if (-not $SkipCheck) {
    Write-Header "4. Nach-Check UI11"
    $checkScript = Join-Path $Base "Scripts\python_runner\check_ui11_register_lesadapter.py"
    python $checkScript
    if ($LASTEXITCODE -ne 0) {
...
$ausgaben = @(
    "Windows_App\Logs\UI11_REGISTER_ADAPTER.json",
    "Windows_App\Logs\UI11_REGISTER_UEBERSICHT.html",
    "Windows_App\Logs\UI11_REGISTER_LESADAPTER_BERICHT.txt",
    "Windows_App\Logs\UI11_KONSISTENZ_LOG.json"
)
...
# 6. Abschluss
Write-Header "UI11 AUTOLAUF ABGESCHLOSSEN"
