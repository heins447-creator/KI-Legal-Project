...
$PyFile = Join-Path $Root "Scripts\python_runner\042_agent_dokumentart_erkennen_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_AGENT_DOKUMENTART_$Ts.txt"
$Failed = $false
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Agent Dokumentart fehlgeschlagen."
    }

    W "AGENT DOKUMENTART abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Agent Dokumentart beendet."
        exit 0
    }
}
