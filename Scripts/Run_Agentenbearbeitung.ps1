...
$PyFile = Join-Path $Root "Scripts\python_runner\040_agentenbearbeitung_grundmodul_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_AGENTENBEARBEITUNG_$Ts.txt"
$Failed = $false
...
try {
    W "AGENTENBEARBEITUNG gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Agentenbearbeitung fehlgeschlagen."
    }

    W "AGENTENBEARBEITUNG abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Agentenbearbeitung beendet."
        exit 0
    }
}
