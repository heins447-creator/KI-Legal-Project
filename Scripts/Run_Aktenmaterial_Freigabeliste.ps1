...
$PyFile = Join-Path $Root "Scripts\python_runner\027_posteingang_aktenmaterial_freigabeliste_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_AKTENMATERIAL_FREIGABELISTE_$Ts.txt"
$Failed = $false
...
try {
    W "AKTENMATERIAL FREIGABELISTE gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if ($Code -ne 0) {
        throw "Aktenmaterial-Freigabeliste fehlgeschlagen. Exitcode: $Code"
    }

    W "AKTENMATERIAL FREIGABELISTE abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Aktenmaterial-Freigabeliste ist erzeugt."
        exit 0
    }
}
