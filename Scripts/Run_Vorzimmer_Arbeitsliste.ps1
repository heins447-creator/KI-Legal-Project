...
$PyFile = Join-Path $Root "Scripts\python_runner\017_vorzimmer_arbeitsliste_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_VORZIMMER_ARBEITSLISTE_$Ts.txt"
$Failed = $false
...
try {
    W "VORZIMMER ARBEITSLISTE gestartet."
    W "Root: $Root"
    W "Report: $Report"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Vorzimmer-Arbeitsliste fehlgeschlagen."
    }

    W "VORZIMMER ARBEITSLISTE abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Vorzimmer-Arbeitsliste ist erzeugt."
    }
}
