...
$PyFile = Join-Path $Root "Scripts\python_runner\038_anwaltvorlage_grundmodul_v1.py"
$LogDir = Join-Path $Root "Windows_App\Logs"
$Ts = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
$Report = Join-Path $LogDir "RUN_ANWALTVORLAGE_$Ts.txt"
$Failed = $false
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Anwaltvorlage fehlgeschlagen."
    }

    W "ANWALTVORLAGE abgeschlossen."
    ...
    if ($Failed) {
        ...
        Write-Host "Anwaltvorlage beendet."
        exit 0
    }
}
