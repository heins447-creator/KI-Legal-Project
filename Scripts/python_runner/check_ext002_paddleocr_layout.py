#!/usr/bin/env python3
"""
EXT-002 – PaddleOCR / Layout-Analyse Check

Ziel:
Pruefung der Voraussetzungen fuer die OCR-Strecke.
"""

import json
import sys
from pathlib import Path

BASE_DIR = Path(__file__).resolve().parents[2]
CONFIG_FILE = BASE_DIR / "Config" / "ext002_paddleocr_layout_v1.json"
DB_PATH = BASE_DIR / "Database" / "DuckDB" / "alin_local.duckdb"

def pruefe() -> tuple[bool, list[str]]:
    fehler = []

    if not CONFIG_FILE.exists():
        fehler.append("Config fehlt")
        return False, fehler

    try:
        json.loads(CONFIG_FILE.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        fehler.append("Config ist ungueltiges JSON")
        return False, fehler

    if not DB_PATH.exists():
        fehler.append("DuckDB-Datenbank nicht gefunden")
        return False, fehler

    try:
        import duckdb
        conn = duckdb.connect(str(DB_PATH))
        tabellen = conn.execute(
            "SELECT table_name FROM information_schema.tables WHERE table_schema = 'alin_ext001' AND table_name IN ('dokumente', 'ocr_ergebnisse')"
        ).fetchall()
        tabellen_namen = {t[0] for t in tabellen}
        if "dokumente" not in tabellen_namen:
            fehler.append("Tabelle 'dokumente' fehlt")
        if "ocr_ergebnisse" not in tabellen_namen:
            fehler.append("Tabelle 'ocr_ergebnisse' fehlt")
        conn.close()
    except Exception as e:
        fehler.append(f"DuckDB-Fehler: {e}")

    # PaddleOCR pruefen (optional – Runner erzeugt Stub wenn nicht vorhanden)
    try:
        import paddleocr
        print("[INFO] PaddleOCR ist installiert")
    except ImportError:
        print("[WARNUNG] PaddleOCR nicht installiert – Runner erzeugt Installations-Skript")

    return (len(fehler) == 0, fehler)

def main() -> int:
    ok, fehler = pruefe()
    if ok:
        print("[OK] EXT-002 Check bestanden")
        return 0
    else:
        for f in fehler:
            print(f"[FEHLER] {f}")
        return 1

if __name__ == "__main__":
    sys.exit(main())
